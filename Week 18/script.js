...
const data = `[
  ...
  /* Probleme : response n'est plus accessible
  ...
  */
  ...
  function createCard({name, universe, alterego, occupation, friends, superpowers, url, info, rating, id}) {
    ...
    majStars(ratingStars, id);
    executeRating(ratingStars, id);   
  }


  function majStars(stars, id) {
    ...
    stars.forEach(star => {
        ...
        let heroID = Number(star.className.match(/[0-9]+/g)[0]);

        if(i < response[heroID].rating) {
            star.className = starClassActive;
        }
        else star.className = starClassInactive;
  ...
  function executeRating(stars, id) {
    ...
    stars.map((star) => {
        star.onclick = () => {
          // i ira de 0 (la premiere etoile) à 4 (la derniere etoile)
          i = stars.indexOf(star);
          id = Number(star.className.match(/[0-9]+/g)[0]);
          if (star.className === starClassInactive) {
            score = i + 1;
            response[id].rating = score;

            localStorage.setItem('data', JSON.stringify(response));
              // Si on selectionne une etoile vide, on remplit toutes les etoiles jusqu'a celle sur laquelle on a cliquee.
            ...
            localStorage.setItem('data', JSON.stringify(response));
            // Si on selectionne une etoile pleine, on vide toutes les etoiles apres celle sur laquelle on a clique.
            for (i = i + 1; i < starsLength; i++) stars[i].className = starClassInactive;
          }
        };
    });
  }